...
export class Laby {
    ...
    private saveDataToFile(): void {
        const filePath = SAVEFILE_PATH + this.saveFileName;
        const dirPath = SAVEFILE_PATH;

        // Ensure the directory exists
        if (!fs.existsSync(dirPath)) {
            fs.mkdirSync(dirPath, { recursive: true });  // Create directories if they don't exist
            this.log.warn("Save path '", dirPath ,"' did not exist, directories created.");
        }
        ...
        this.log.debug("Data successfully saved as '", filePath,"'.");
    }
    ...
    private loadDataFromFile(): void {
        ...
        if (!fs.existsSync(filePath)) {
            this.log.error("Save file '",filePath,"' not found.");
            return;
        }
